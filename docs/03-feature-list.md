...
| A5 | **Live pose guidance** — bullseye bubble level, plain-language "tip it this way" prompts, haptic on arrival, graded not binary | P0 | A | Numbers are unreadable handheld. Tolerances set by what the maths requires (see below), not by what feels rigorous. Implemented in [level.js](../prototype/lib/level.js) |
| A5a | **Steadiness detection + auto-capture on settle** | P0 | A | Holding still *and* pressing a button is the hard part of handheld capture. Movement at the shutter ruins more captures than tilt ever does |
| A5b | **Subject modes** — wall (phone upright) vs ground/paving (phone down) vs free | P1 | A | The paving and kerb shots the photo-survey contract asks for need a different "level" |
